...
<div class="container py-5" id="produk-unggulan">
  ...
  <div class="row g-4">
    ...
    <div class="col-md-3">
      <div class="card product-card h-100 shadow-sm" role="button" data-bs-toggle="modal" data-bs-target="#popup-{{ $barang->id_barang }}">
        
        {{-- Badge untuk stok & best seller --}}
        @if($barang->stok == 0)
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ $barang->nama_barang }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <div class="row">
              <div class="col-md-6">
                <img src="{{ asset('images/'.$barang->gambar) }}" class="img-fluid rounded mb-3">
              </div>
              <div class="col-md-6">
                <p>{{ $barang->deskripsi }}</p>
                <p><strong>Harga:</strong> Rp {{ number_format($barang->harga) }}/{{ $barang->satuan }}</p>
                ...
                @else
                  ...
                  @auth('pelanggan')
                    ...
                    <form method="POST" action="{{ route('keranjang.tambah', $barang->id_barang) }}">
                      @csrf
                      <div class="mb-3">
                        <label for="jumlah-{{ $barang->id_barang }}">Jumlah</label>
                        <input type="number" name="jumlah" id="jumlah-{{ $barang->id_barang }}" 
                               class="form-control" min="1" max="{{ $barang->stok }}" value="1" required>
                      </div>
  ...
  <div class="text-center mt-5">
    <a href="{{ route('dashboard') }}" class="btn btn-lg btn-gradient">
      Lihat Semua Produk →
    </a>
  </div>
</div>
